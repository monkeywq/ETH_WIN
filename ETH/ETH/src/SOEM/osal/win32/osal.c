...
int osal_getrelativetime(struct timeval *tv, void *tz)
{
...
{
   *(HANDLE *)thandle = CreateThread(NULL, stacksize, func, param, 0, NULL);
   if(!*(HANDLE *)thandle)
   {
      return 0;
   }
...
int osal_thread_create_rt(void *thandle, int stacksize, void *func, void *param)
{
   ...
   if (ret)
   {
      ret = SetThreadPriority(*(HANDLE *)thandle, THREAD_PRIORITY_TIME_CRITICAL);
   }
   return ret;
}
